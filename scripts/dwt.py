...
def embed(cover_img, watermark_img, output_watermarked_img, k=0.05):
    # read cover
    cover_img = Image.open(cover_img).convert('L')
    data = np.array(cover_img)

    # read watermark
    watermark_img = Image.open(watermark_img)
    watermark_data = np.array(watermark_img)
    ...
    if watermark_data.size > HL3.size:
        ...
        watermark_resized = watermark_img.resize((HL3.shape[1], HL3.shape[0]))
        # tạo file resize mới
        idx_rdot = watermark_img.rfind('.')
        extension = watermark_img[idx_rdot:]
        name = watermark_img[:idx_rdot]
...
def extract(origin_image, watermarked_image, watermark_image, extract_watermarked_img, k=0.05):
    # read origin image
    origin_img = Image.open(origin_image)
    ...
    # get width height watermark img
    height_w, width_w = watermark_data.shape
